...
int instruction(struct item **Ptr){
	...
	do{	
		...
		for(int x = 0; x < strlen(Input); x++){
			fail += !(isdigit(Input[x]));
		}

		if(!(input >= 1 && input <= 6) || fail != 0){
...
int append(struct item **Ptr){
	...
	// to take on the extra key stroke
  	scanf("%c", husk);

  	// inputting the valid data into a temp node for adding
  	// getting the id. (id)
  	while(fail != 0 || !strcmp(string, "")){
    		fail = 0;
